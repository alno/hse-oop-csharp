...
namespace StarSystem
{
    public partial class SpaceViewForm : Form
    {
        ...
        private void SelectBody(CelestialBody body)
        {
            ...
            {
                Control c = CreateBodyControl(selectedBody);
                c.Top = 0;                
                c.Left = 0;
                c.Width = splitContainer.Panel2.ClientSize.Width;
                c.Height = splitContainer.Panel2.ClientSize.Height;
        ...
        private Control CreateBodyControl(CelestialBody body)
        {
            if (body is Planet)
            {
                return new PlanetViewer( (Planet) body );
            }
            else
            {
                return new Panel();
