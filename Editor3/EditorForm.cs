...
namespace Editor1
{
    public partial class EditorForm : Form
    {
        ...
        private void exportToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            dialog.FileName = "MyDrawing.bmp";
            dialog.Filter = "Bitmap (*.bmp)|*.bmp|Scalable Vector Graphics (*.svg)|*.svg";

            if (dialog.ShowDialog() == DialogResult.OK) // Показываем диалог
            {
                switch (dialog.FilterIndex) // Определяем тип выбранного файла
                {
                    case 1:
                        ExportToBmp(dialog.FileName);
                        break;
                    case 2:
                        ExportToSvg(dialog.FileName);
                        break;
                    default:
                        MessageBox.Show("Unknown file format " + dialog.FilterIndex );
                        break;
                }
            }
        }
        ...
        // Экспорт в BMP
        private void ExportToBmp(string fileName)
        {            
            Bitmap bitmap = new Bitmap(splitContainer.Panel1.ClientRectangle.Width, splitContainer.Panel1.ClientRectangle.Height); // Создаем новый битмап

            splitContainer.Panel1.DrawToBitmap(bitmap, splitContainer.Panel1.ClientRectangle); // Рисуем на нем содержимое панели

            bitmap.Save(fileName, ImageFormat.Bmp); // Сохраняем в формате BMP
        }

        // Экспорт в SVG
        private void ExportToSvg(string fileName)
        {
            XmlDocument svgDocument = new XmlDocument(); // Создаем XML-документ

            XmlElement svgElement = svgDocument.CreateElement("svg"); // Создаем корневой элемент
            svgElement.SetAttribute("version", "1.1"); // Версия формата
            svgElement.SetAttribute("xmlns", "http://www.w3.org/2000/svg"); // Пространство имен
            svgElement.SetAttribute("width", Convert.ToString(splitContainer.Panel1.ClientSize.Width)); // Ширина
            svgElement.SetAttribute("height", Convert.ToString(splitContainer.Panel1.ClientSize.Height)); // Высота

            foreach ( Element el in elements )
                svgElement.AppendChild(el.ExportToSvg(svgDocument)); // Создаем элемент для каждой фигуры

            svgDocument.AppendChild(svgElement);
            svgDocument.Save(fileName); // Записываем документ в файл
        }
